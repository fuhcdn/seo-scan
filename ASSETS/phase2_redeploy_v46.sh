#!/usr/bin/env bash
# Phase 2 Production Re-Deploy Card v4.6 — Fail-Closed Single Orchestrator with Global ERR Routing
set -Eeuo pipefail
...
export EXPECTED_H1_JSON='"ok": *true'
...
# ───────── restore-first rollback handler (non-recursive) ─────────
restore_and_rollback() {
  local rc="${1:-1}"
  if [ "$IN_ROLLBACK" = "1" ]; then
    echo "ROLLBACK-ALREADY-RUNNING — preserving evidence, exiting with original rc=$rc"
    exit "$rc"
  fi
  IN_ROLLBACK=1
  set +e
  ...
  echo "LEGACY-PUBLIC-HEALTH: $h2" | tee -a "$FAIL_LOG"
  ...
  echo "ROLLBACK-COMPLETE — failure evidence preserved in $FAIL_LOG"
  exit "$rc"
}

# ───────── global ERR trap (routes all unhandled post-backup errors) ─────────
global_err_handler() {
  local exit_code="$?"
...
# ───────── verify_h1_h5 (fail-closed, auto-rollback via ERR trap / explicit calls) ─────────
verify_h1_h5() {
