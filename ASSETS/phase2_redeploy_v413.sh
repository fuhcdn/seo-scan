#!/usr/bin/env bash
# Phase 2 Production Re-Deploy Card v4.13 — Fail-Closed Orchestrator, Final (truthful rollback terminal state)
set -Eeuo pipefail
...
export LEGACY_IMAGE_ID='sha256:d5dd387df73cd698f4e1b3ba870ef1717b4dee86d087700deded25b34d50d36f'
...
export EXPECTED_H6_SUMMARY='DRY-RUN SUITE: 16/16 PASS'
...
# dedicated immutable rollback tag must also resolve to legacy image ID
docker image inspect "seoscanaudit/legacy-rollback:rebuilt-20260915-191500" --format '{{.Id}}' > /dev/null 2>&1   || preflight_fail "PF4-ROLLBACK-TAG-ABSENT"
[ "$(docker image inspect "seoscanaudit/legacy-rollback:rebuilt-20260915-191500" --format '{{.Id}}')" = "$LEGACY_IMAGE_ID" ]   || preflight_fail "PF4-ROLLBACK-TAG-MISMATCH"
...
echo "H6 REQUIRED CASES 12/12 PASS —"
echo "H6 EXTRA ASSERTIONS 4/4 PASS —"
echo "H6 TOTAL 16/16 PASS —"
echo "RESEND 0 —"
