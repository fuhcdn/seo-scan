#!/usr/bin/env bash
# Phase 2 Production Re-Deploy Card v4.9 — Fail-Closed Orchestrator, Final (truthful rollback terminal state)
set -Eeuo pipefail
...
restore_and_rollback() {
  ...
  # Terminal state: clean rollback requires RESTORE_RESULT=OK AND MANIFEST_RESULT=MATCH
  # AND legacy health AND outbound absence — all independently verified.
  ...
  elif [ "$legacy_ok" = "0" ] && [ "$restore_ok" = "1" ]; then
